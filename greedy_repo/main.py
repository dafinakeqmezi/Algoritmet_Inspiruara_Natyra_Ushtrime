...
def main():
    ...
    parser_arg.add_argument(
        ...
        dest="scheduler",
        choices=["1", "2", "3", "4", "5", "6"],
        help="Scheduler to use: 1=Greedy, 2=GreedyLookahead, 3=Beam, 4=BeamAdvanced, 5=UpperBound, 6=BeyondDynamicBeam"
    )
    parser_arg.add_argument("--beam-width", type=int, default=3, help="Base beam width for beam schedulers")
    parser_arg.add_argument("--jump-cap", type=int, default=30, help="Jump cap in minutes for beam search")
    ...
    parser_arg.add_argument("--max-beam-multiplier", type=int, default=3, help="Max multiplier for iterative deepening")
    parser_arg.add_argument("--time-limit", type=float, default=None, help="Time limit in seconds")
    ...
    print("3: Beam Search bounded lookahead")
    print("4: Beam Search Advanced")
    print("5: Upper Bound")
    print("6: Beyond Dynamic Beam Search + Iterative Deepening + Advanced Backtracking")

    choice = args.scheduler if args.scheduler else input("Select scheduler [1/2/3/4/5/6] default 1: ").strip() or "1"

    run_solution_later = True
    ...
    if choice == "1":
        ...
        max_beam_multiplier = args.max_beam_multiplier
        ...
        if time_limit:
            solution = scheduler.generate_solution_with_time(
                restarts=restarts,
                time_limit=time_limit
            )
        else:
        ...
        run_solution_later = False

    else:
        scheduler = GreedyScheduler(instance)

    if run_solution_later:
